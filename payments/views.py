...
def capture_paypal_order(request):
    if request.method == "POST":
        try:
            ...
            # Beispiel: Extrahiere die Zahlungsmethode aus der Antwort
            payment_source = "paypal"  # Default fallback
            try:
                capture = paypal_capture_data["purchase_units"][0]["payments"]["captures"][0]
                payment_source_data = capture.get("payment_source", {})
                if "card" in payment_source_data:
                    payment_source = "kreditkarte"
                elif "bank_account" in payment_source_data:
                    payment_source = "sepa"
                elif "paypal" in payment_source_data:
                    payment_source = "paypal"
            except (KeyError, IndexError):
                pass  # fallback bleibt "paypal"

            payment_status = paypal_capture_data.get("status")

            if payment_status == "COMPLETED":
                anmeldung_obj.ist_bezahlt = True
                anmeldung_obj.bezahlmethode = payment_source  # <-- Hier speichern
            ...
            return JsonResponse(
                {"error": f"Fehler beim Abschließen der PayPal-Bestellung: {error_details}"}, status=500
            )
        except Exception as e:
