    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                logger.error(
                    f"Mehrere Anmeldungen gefunden für PayPal Order ID: {order_id}. Dies sollte nicht passieren!"
                )
                return JsonResponse(
                    {
            ...
            if payment_status == "COMPLETED":
                ...
                anmeldung_obj.zahlungsdatum = (
                    datetime.datetime.now()
                )  # Aktuelles Datum/Uhrzeit
                anmeldung_obj.save()
                logger.info(
                    f"Anmeldung {anmeldung_obj.id} erfolgreich bezahlt durch PayPal Order {order_id}. Status: COMPLETED."
                )
            else:
                ...
                logger.warning(
                    f"PayPal Order {order_id} hat Status {payment_status}. Anmeldung {anmeldung_obj.id} nicht als bezahlt markiert."
                )
                anmeldung_obj.ist_bezahlt = (
                    False  # Sicherstellen, dass es auf False bleibt
                )
